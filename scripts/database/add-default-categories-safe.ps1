...
$backupFile = Join-Path $backupDirectory "finance-tracker-pre-add-categories-$timestamp.dump"
...
try {
  ...
  $env:DATABASE_CHANGE_CONFIRMED = 'ADD_DEFAULT_CATEGORIES'
  $env:DATABASE_BACKUP_FILE = $backupFile
  $env:DATABASE_BACKUP_SHA256 = $backupHash

  Push-Location $repositoryRoot
  try {
    & pnpm exec tsx --env-file=.env.local scripts/database/add-default-categories.ts
    if ($LASTEXITCODE -ne 0) {
      throw 'The category change failed. The verified backup remains available.'
    }
  } finally {
    Pop-Location
  }
  ...
  Write-Output 'Production categories were inserted or already existed and were verified.'
} finally {
