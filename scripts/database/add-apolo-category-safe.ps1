...
$backupFile = Join-Path $backupDirectory "finance-tracker-pre-add-apolo-category-$timestamp.dump"
...
try {
  ...
  if (-not ($backupContents -match 'TABLE DATA public merchant_rules')) {
    throw 'The backup does not contain public.merchant_rules data. The production change was not executed.'
  }
  ...
  $env:DATABASE_CHANGE_CONFIRMED = 'ADD_APOLO_CATEGORY'
  $env:DATABASE_BACKUP_FILE = $backupFile
  $env:DATABASE_BACKUP_SHA256 = $backupHash

  Push-Location $repositoryRoot
  try {
    & pnpm exec tsx --env-file=.env.local scripts/database/add-apolo-category.ts
    if ($LASTEXITCODE -ne 0) {
      throw 'The Apolo category change failed. The verified backup remains available.'
    }
  } finally {
    Pop-Location
  }
  ...
  Write-Output 'Apolo category was inserted or already existed and was verified.'
} finally {
